    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="container flex items-center justify-between">
        ...
        <NavLink to="/" className="flex items-center gap-2 group" onClick={closeMenu}>
          <BookOpen className="text-accent-gold group-hover:rotate-6 transition-transform" size={24} />
            ...
            {links.map((link) => (
              <li key={link.path}>
                <NavLink
                  to={link.path}
                  className={({ isActive }) =>
                    `text-sm font-medium relative py-1 transition-colors hover:text-accent-gold ${
                      isActive 
          ...
          {links.map((link) => (
            <li key={link.path} className="w-full">
              <NavLink
                ...
                onClick={closeMenu}
                className={({ isActive }) =>
                  `block text-base font-medium py-2 border-b border-slate-100 dark:border-slate-800 transition-colors ${
                    isActive 
                      ? 'text-accent-gold dark:text-accent-gold font-semibold' 
      ...
      {isOpen && (
        <div 
          onClick={closeMenu} 
          className="fixed inset-0 z-30 bg-black/20 dark:bg-black/40 backdrop-blur-sm md:hidden"
        />
      )}
    </nav>
  );
